...
def parse_esp_range(text: str):
    """从 Multiwfn 模块12 输出解析 ESP 表面 min/max 值。
    Returns: (min_val, max_val, unit) 或 (None, None, None)
    """
    m_kcal = re.search(
    ...
    if m_kcal:
        return float(m_kcal.group(1)), float(m_kcal.group(2)), "kcal/mol"
    m_min_au = re.search(r"Global surface minimum:\s+([\d\.\-+]+)\s*a\.u\.", text)
    m_max_au = re.search(r"Global surface maximum:\s+([\d\.\-+]+)\s*a\.u\.", text)
    if m_min_au and m_max_au:
        return float(m_min_au.group(1)), float(m_max_au.group(1)), "a.u."
