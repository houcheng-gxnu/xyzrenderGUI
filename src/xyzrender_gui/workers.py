...
from .config import CMD_ESPISO, CMD_ESP_QUERY_RANGE, CHARGE_TYPES, _filter_mw_line, _parse_mw_progress
    ...
    def run(self):
        ...
        try:
            proc = subprocess.Popen(
                [self._mw_exe, fch_name, "-ESPrhoiso", "0.001"],
                stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                cwd=self._work_dir, text=True, bufsize=1,
            )
            proc.stdin.write(CMD_ESPISO)
            proc.stdin.close()
        ...
        if proc.returncode not in (0, 24):
            ...
            tail = full_out[-2000:] if len(full_out) > 2000 else full_out
...
class MOCubeWorker(QThread):
    ...
    def run(self):
        ...
        work_dir = tempfile.mkdtemp(prefix="_mo_", suffix=f"_{self._orb}")
        ...
        if not os.path.exists(mo_cube):
            tail = proc.stdout[-2000:] if proc.stdout else "(无输出)"
            self.status_signal.emit(f"MOvalue.cub 未生成 (code={proc.returncode})\n---\n{tail}\n---")
            self.finished_signal.emit(False, "")
            return
...
class IgmhWorker(QThread):
    ...
    def run(self):
        ...
        fch_dir = os.path.dirname(os.path.abspath(self._fch))
        stem = os.path.splitext(fch_name)[0]
        work_dir = tempfile.mkdtemp(prefix="_igmh_", suffix=f"_{stem}")
...
class DetectRangeWorker(QThread):
    """后台运行 Multiwfn 查询 ESP 表面范围 (模块12 快速分析)。"""

    finished_signal = pyqtSignal(str)  # stdout

    def __init__(self, multiwfn_exe, fch_path):
        super().__init__()
        self._mw_exe = multiwfn_exe
        self._fch = fch_path
    ...
    def run(self):
        ...
        try:
            proc = subprocess.run(
                ...
                input=CMD_ESP_QUERY_RANGE,
                capture_output=True,
                timeout=600,
                encoding="utf-8", errors="replace",
            )
            self.finished_signal.emit(proc.stdout)
        except Exception:
            self.finished_signal.emit("")
