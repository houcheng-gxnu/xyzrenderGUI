...
基于 PyQt5 的分子结构与等值面可视化 GUI，将 `xyzrender` 三维分子渲染引擎与 `Multiwfn` 量子化学分析工具整合为一体化的交互式应用。
...
- **Python** ≥ 3.10
- **PyQt5** — GUI 框架
- **xyzrender** — 分子结构渲染引擎（SVG 输出）
- **molcanvas** — 轻量级 QPainter 分子画布（实时 3D 预览）
- **Multiwfn** — 量子化学波函数分析（ESP / MO / IGMH / 电荷计算）

## 安装

```bash
# 克隆仓库
git clone https://cnb.cool/chem311/xyzrenderGUI.git
cd xyzrenderGUI

# 安装依赖 (需预先安装 xyzrender 和 molcanvas)
pip install PyQt5
```

### Multiwfn 配置

下载 [Multiwfn](http://sobereva.com/multiwfn/) 后，启动程序在 **文件** 栏中浏览选择 `Multiwfn.exe` 路径，配置将自动保存至 `xyzrender_viewer.ini`。
